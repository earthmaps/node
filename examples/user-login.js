...
app.get('/', function(req, res){
	if( req.session.user ){
		...
		//var auth =
		res.send('<html><body><h2>Login</h2><form action="/auth/earthmaps/password"><input name="username" placeholder="username"><input name="password" placeholder="password"><input type="submit"></form></body></html>');
	}
	res.end();
});


app.get('/auth/earthmaps/password', password);
...
function password( req, res, next ){
	var data = req.query;
	earthmaps.login( data, function( err, result ){
