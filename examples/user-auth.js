...
app.get('/', function(req, res){
	if( req.user ){
		...
		//var auth =
		res.send('<html><body><a href="/auth/earthmaps">Login</a></body></html>');
	}
	res.end();
});


app.get('/auth/earthmaps', earthmaps.auth());

app.get('/auth/earthmaps/callback', earthmaps.auth({ successRedirect: '/', failureRedirect: '/'}) );
